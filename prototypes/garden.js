/* ============================================================
   Jardim — 7 estágios ilustrados da árvore + herbário
   ============================================================ */

/* paleta da ilustração (coesa com os tokens) */
const C = {
  bark: '#7A5A3F',
  barkD: '#5E4632',
  stem: '#5E7B43',
...
/* chão compartilhado por todos os estágios */
const GROUND = `
...
const leaf = (cx, cy, deg, fill, rx = 11, ry = 5) =>
...
/* canopy de duas a três camadas (profundidade) */
const canopy = (cx, cy, s) => `
...
function renderTree(i, animate = true) {
  const stage = STAGES[i];
  const apply = () => {
    treeArt.innerHTML = stage.art;
    streakNum.textContent = stage.streak;
    stageName.textContent = stage.name;
    stageHint.textContent = stage.hint;
    treeArt.classList.remove('is-changing');
  };
  if (animate && !reduceMotion) {
    treeArt.classList.add('is-changing');
    setTimeout(apply, 280);
  } else {
    apply();
  }
  [...chipsWrap.children].forEach((c, ci) => c.classList.toggle('is-active', ci === i));
  current = i;
}
...
const weekly = [
  { name: 'Semana 1', date: '12 mai', earned: true },
  { name: 'Semana 2', date: '19 mai', earned: true },
  { name: 'Semana 3', date: '26 mai', earned: true },
  { name: 'Semana 4', date: '02 jun', earned: true },
  { name: 'Semana 5', date: 'A conquistar', earned: false },
  { name: 'Semana 6', date: 'A conquistar', earned: false },
];
const monthly = [
  { name: 'Mês 1', date: '31 mai', earned: true },
  { name: 'Mês 2', date: 'A conquistar', earned: false },
  { name: 'Mês 3', date: 'A conquistar', earned: false },
];

function card(item, stampFn) {
  return `<div class="herb-card${item.earned ? '' : ' herb-card--locked'}">
    <div class="herb-card__art">${stampFn(item.earned)}</div>
    <span class="herb-card__name">${item.name}</span>
    <span class="herb-card__date">${item.date}</span>
  </div>`;
}

document.getElementById('weeklyGrid').innerHTML = weekly.map((w) => card(w, weeklyStamp)).join('');
document.getElementById('monthlyGrid').innerHTML = monthly
  .map((m) => card(m, monthlyStamp))
  .join('');
document.getElementById('weeklyCount').textContent =
  `${weekly.filter((w) => w.earned).length} de ${weekly.length}`;
document.getElementById('monthlyCount').textContent =
  `${monthly.filter((m) => m.earned).length} de ${monthly.length}`;
