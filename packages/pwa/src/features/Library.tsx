...
/** Biblioteca pessoal de anotações, por mês, com busca e editor por cima. */
export function Library({ onBack }: { onBack: () => void }) {
  ...
  const [editing, setEditing] = useState<{ devotionalId: string; dateLabel: string } | null>(null);
  const { top, open, close } = useScreenStack<'note'>();
  ...
  const openEditor = (devotionalId: string, dateLabel: string) => {
    setEditing({ devotionalId, dateLabel });
    open('note');
  };
  ...
  return (
    <>
      ...
      {top === 'note' && editing && (
        <NoteEditorScreen
          devotionalId={editing.devotionalId}
