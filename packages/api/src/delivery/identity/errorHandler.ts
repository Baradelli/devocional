...
import { NotificationError } from '../../application/notifications/errors.js';
...
import { notificationErrorResponse } from '../notifications/errorMessages.js';
...
export function identityErrorHandler(
  ...
  if (error instanceof NotificationError) {
    const mapped = notificationErrorResponse(error.code);
    void reply.status(mapped.status).send({ error: error.code, message: mapped.message });
    return;
  }
